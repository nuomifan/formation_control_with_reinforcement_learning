# -*- coding: utf-8 -*-
"""
Created on Sun Apr 19 14:52:36 2020
...
def generate(memory_size):
    memory = deque()
    ob = np.random.randint(0, 2000, (memory_size, 18))
    for s in ob:
        v, action = move(s)
        s_ = s + v
        reward = 1
...
def build_net():
    ...
    model.add(layers.Dense(50, kernel_initializer='random_uniform',
                           bias_initializer='zeros'))
    model.add(layers.Dense(50, kernel_initializer='random_uniform',
                           bias_initializer='zeros'))
...
for j in range(train_times):
    if j % 2 == 0:
        model.set_weights(model_e.get_weights())
    print('round: ', j + 1)
    Q = model_e.predict(ob / 2000)
    Q_next = model.predict(ob / 2000)
    ...
    model_e.fit(ob / 2000, Q, batch_size=batch_size)
...
for i in range(test_size - 1):
    v1, _ = move(ob_t1[i, :])
    action = np.argmax(model.predict((ob_t2[i:i + 1, :] + 2000) / 4000))
    v2 = v[action]
